...
const Review = () => {
  ...
  useEffect(() => {
    ...
    const fakeProduct = {
      ...
      description: "Máy tính chơi game mạnh mẽ với chip i9, RAM 32GB.",
  ...
  if (loading) return <div className="text-center py-10 text-blue-600">Đang tải...</div>;

  return (
    <div className="max-w-lg mx-auto bg-white shadow-lg rounded-lg p-6 mt-10">
      <h2 className="text-2xl font-bold text-gray-800">{product.name}</h2>
      <p className="text-gray-600 mt-2">{product.description}</p>
      <img src={product.qr_code} alt="QR Code" className="mt-4 mx-auto w-32 h-32" />
      <p className="mt-2 text-sm text-gray-500">Mã sản phẩm: {product._id}</p>
    </div>
  );
};
